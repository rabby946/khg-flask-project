...
def get_current_member():
    """Fetch logged-in member from session"""
...
def login():
    if request.method == "POST":
        ...
        member = Member.query.filter_by(nid=nid).first()
        if member and member.password_hash == password:
...
def edit_profile():
    ...
    if request.method == "POST":
        ...
        # Check for uniqueness, excluding the current member
        ...
        # Update member info in the database object
        member.name = name
...
def mark_all_notifications_read():
    ...
    # Bulk update all unread notifications for this member
    Notification.query.filter_by(member_id=member.member_id, is_read=False).update(
        {"is_read": True}
    )
    db.session.commit()
...
def voting():
    ...
    if request.method == "POST":
        ...
        try:
            # Try to convert form values to integers immediately.
